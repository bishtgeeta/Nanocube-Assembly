...
#######################################################################
# DATA PROCESSSING
# 1. READ THE SEGMENTED FILES AND STORE THEM FRAME-WISE IN H5 FILE
#######################################################################
...
for frame in procFrameList[rank]:
    gImgSegmented = fp['/dataProcessing/gImgSegmented/'+str(frame).zfill(zfillVal)].value
    finalImage = numpy.column_stack((gImgSegmented, gImgSegmented))
